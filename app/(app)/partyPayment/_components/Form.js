...
import { createPartyMaster } from '@/app/_actions/createPartyMaster';
import toast from 'react-hot-toast';
import { getMemoById } from '@/app/_actions/getMemoById';
import { createPartyPayment } from '@/app/_actions/createPartyPayment';
...
function Form() {
  ...
  const onSearch = async (data) => {
    ...
    const result = await getMemoById(data.id, 1);
  ...
  const onConfirm = async (data) => {
    ...
    if (
      data.type === 'advance' &&
      memo.party_net_balance === null &&
      Number(data.amount) > Number(memo.party_freight_charges)
    ) {
      toast.dismiss();
    ...
    if (
      data.type === 'advance' &&
      Number(memo.party_freight_charges) === Number(memo.party_net_balance) &&
      Number(data.amount) > Number(memo.party_freight_charges)
    ) {
      toast.dismiss();
      toast.error(`❌ Amount Bigger than Charges`);
    ...
    if (
      ...
      memo.party_net_balance !== null &&
      Number(memo.party_freight_charges) > Number(memo.party_net_balance) &&
      Number(data.amount) > Number(memo.party_net_balance)
    ) {
      toast.dismiss();
    ...
    const newData = {
      ...
      amount: data?.amount,
      memo_id: memo?.id,
      party_id: memo?.party_master?.id,
      mode_of_payment: data?.mode_of_payment,
      remarks: data?.remarks,
      date: formattedDate(data?.date),
    };

    const result = await createPartyPayment(newData);
    ...
    if (result?.error) {
      ...
      const updatedMemo = await getMemoById(memo.id, 1);
          ...
          <div className="flex justify-center mt-10">
            <div className="flex text-primary-100 font-semibold text-xl border-2 border-primary-100 rounded-2xl w-5xl px-5 py-5">
              <div className="flex-1 ml-10 space-y-2">
                ...
                />
                <H1
                  label="Party Net Balance"
                  text={`₹ ${formatINR.format(
                    memo?.party_net_balance === null
                      ? memo?.party_freight_charges
                      : memo?.party_net_balance,
                  )}`}
                />
            ...
            <div className="p-5 w-xl border-2 border-primary-100 rounded-2xl">
              ...
              <div className="flex items-center justify-between">
                ...
                <select
                  ...
                  className="mt-8 w-60 h-12 p-2 text-xl rounded-lg border-4 border-primary-100 bg-transparent text-primary-100 outline-none transition-colors focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20"
                >
