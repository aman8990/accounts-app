...
function Payments({ payments }) {
  const [partyPayments, setPartyPayments] = useState(payments);
  ...
  const onSearch = async (data) => {
    ...
    const result = await getCustomPayments(1, date);

    if (result?.error) {
      toast.error(`❌ ${result.error}`);
    } else {
      toast.success('Success');
      setPartyPayments(result);
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await deletePayment(paymentToDelete.id, 1);

      setPartyPayments((current) =>
              ...
              {partyPayments?.map((payment) => (
                ...
                >
                  ...
                  <td className="p-3 w-80 text-center">
                    {payment?.party_master?.full_name ?? '-'}
                  </td>
